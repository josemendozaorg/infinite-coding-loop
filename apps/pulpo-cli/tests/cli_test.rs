use anyhow::Result;
use pulpo_engine::{agents::cli_client::ShellCliClient, orchestrator::Orchestrator};
...
async fn test_orchestration_cycle() -> Result<()> {
    ...
    orchestrator.executor.graph = pulpo_engine::graph::DependencyGraph::load_from_metamodel(
        &metamodel_json,
        Some(&ontology_path),
    )?;
