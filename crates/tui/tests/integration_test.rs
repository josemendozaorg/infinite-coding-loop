...
async fn test_happy_path_calculator_generation() {
    ...
    loop {
        ...
        for m in missions {
            for t in m.tasks {
                if t.status == TaskStatus::Pending {
                    let w_name = t.assigned_worker.clone().unwrap_or_else(|| "Bot".to_string());
        ...
        if let Some((mid, tid, worker_name)) = pending_task {
             ...
             let worker: Box<dyn ifcl_core::Worker> = if w_lower.contains("gemini") || w_lower.contains("planner") || w_lower.contains("git") {
             ...
             let _ = orchestrator.execute_task(Arc::clone(&bus), mid, tid, worker.as_ref()).await;
        } else if all_done {
    ...
    fn find_cargo_project(dir: &std::path::Path) -> Option<(std::path::PathBuf, std::path::PathBuf)> {
         if dir.is_dir() {
    ...
    let (cargo_toml_path, main_rs_path) = find_cargo_project(std::path::Path::new(test_workspace_path))
    ...
    if main_rs_path.exists() { found_main = true; }
    if cargo_toml_path.exists() { found_cargo = true; }

    if !found_main {
        println!("WARNING: src/main.rs not found immediately. Listing workspace:");
        let _ = std::process::Command::new("ls").arg("-R").arg(test_workspace_path).status();
    }
    ...
    assert!(main_content.contains("fn main"), "main.rs does not contain 'fn main'");
