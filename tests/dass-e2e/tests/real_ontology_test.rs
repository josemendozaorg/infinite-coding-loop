...
async fn test_small_ontology_workflow() -> Result<()> {
    ...
    let base_ontology_path = root_dir.join("ontologies/software-engineering");

    // We use mini_ontology for the graph (Small, Controlled)
    let mini_ontology_path =
        manifest_dir.join("../../tests/dass-e2e/fixtures/mini_ontology/ontology.json");
