...
async fn test_small_ontology_workflow() -> Result<()> {
    ...
    let base_ontology_path = root_dir.join("pulpo-ontologies/software-engineering");

    // We use mini_pulpo_ontology for the graph (Small, Controlled)
    let mini_pulpo_ontology_path =
        manifest_dir.join("../../tests/pulpo-e2e/fixtures/mini_pulpo_ontology/ontology.json");
    let ontology_json = std::fs::read_to_string(&mini_pulpo_ontology_path)
        .expect("Failed to read mini_pulpo_ontology/ontology.json");
