use ratatui::{
    ...
    widgets::{Block, Borders, List, ListItem, Paragraph, Table, Row, Cell, Clear, Wrap, Gauge},
    Frame,
};
use ifcl_core::{AppMode, MenuAction, LoopStatus, WorkerRole, TaskStatus, AiProvider, WizardStep, LogPayload, ThoughtPayload};
...
pub fn render_node_recursive(
    ...
    let children: Vec<_> = graph.neighbors_directed(node_idx, petgraph::Direction::Outgoing).collect();
    let count = children.len();
    for (i, child_idx) in children.into_iter().enumerate() {
        render_node_recursive(graph, child_idx, depth + 1, &new_prefix, i == count - 1, items, visited);
    }
}
